...
const iconDir=path.join(resDir,'drawable-nodpi');fs.mkdirSync(iconDir,{recursive:true});const output=path.join(iconDir,'launcher_icon.png');
...
write(path.join(androidDir,'app/src/main/res/values/colors.xml'),`<resources><color name="splash_bg">#09090B</color></resources>`);
...
write(path.join(androidDir,'app/src/main/java/com/lanzy/ziptoapk/MainActivity.java'),`package com.lanzy.ziptoapk;import android.app.Activity;import android.os.Bundle;import android.webkit.WebView;import android.webkit.WebViewClient;public class MainActivity extends Activity{public void onCreate(Bundle b){super.onCreate(b);WebView w=new WebView(this);w.setBackgroundColor(0xFF09090B);w.setWebViewClient(new WebViewClient());w.getSettings().setJavaScriptEnabled(true);w.getSettings().setDomStorageEnabled(true);w.loadUrl("file:///android_asset/www/index.html");setContentView(w);}}`);
// Debug builds are automatically signed by the Android Gradle plugin and are installable without a private signing key.
